  ...
  def runner(): ChainBuilder = {
    exec(addCookie(Cookie("connect.sid", ConfigLoader.sessionID)))
      ...
      .exec(http("HTTP: Confirm SID (48hr)")
        .get("/socket.io/?EIO=3&transport=polling&t=" + "LDX" + RandomGenerator.string(4) + "&sid=${sid}")
        .check(status.is(200)))
        .exec(http("HTTP: POST Subscribe to article (48hr)")
          .post("/socket.io/?EIO=3&transport=polling&t=" + "LDX" + RandomGenerator.string(4) + "&sid=${sid}")
