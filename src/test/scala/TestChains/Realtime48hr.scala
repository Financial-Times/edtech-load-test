  ...
  def runner(): ChainBuilder = {
    exec(addCookie(Cookie("connect.sid", ConfigLoader.sessionID)))
      ...
      .exec(http("HTTP: Get SID")
        .get("/socket.io/?EIO=3&transport=polling&t=" + "LB2" + RandomGenerator.string(4))
        .check(regex("\"sid\":\"(.*)\",\".*").saveAs("sid"))
        .check(status.is(200)))
      .exec(http("HTTP: Confirm SID")
        .get("/socket.io/?EIO=3&transport=polling&t=" + "LB2" + RandomGenerator.string(4) + "&sid=${sid}")
        .check(status.is(200)))
      .exec(http("HTTP: POST Subscribe to article")
        .post("/socket.io/?EIO=3&transport=polling&t=" + "LB2" + RandomGenerator.string(4) + "&sid=${sid}")
        .body(StringBody("88:42[\"subscribeToArticle\",{\"uuid\":\"${uuid}\",\"timespan\":\"48h\"}]"))
        .check(status.is(200)))
      ...
      .exec(http("HTTP: POST Subscribe to article")
        .post("/socket.io/?EIO=3&transport=polling&t=" + "LB2" + RandomGenerator.string(4) + "&sid=${sid}")
        .body(StringBody("33:42[\"unsubscribeFromArticle\",null]88:42[\"subscribeToArticle\",{\"uuid\":\"${uuid}\",\"timespan\":\"48h\"}]"))
        .check(status.is(200)))
